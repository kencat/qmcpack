...
";
  ...
  ValueType ratio_0 = std::exp(j3->evalLogRatio(elec_, 0));
  elec_.rejectMove(0);

  elec_.makeMove(1, newpos - elec_.R[1]);
  ValueType ratio_1 = std::exp(j3->evalLogRatio(elec_, 1));
  elec_.rejectMove(1);

  elec_.makeMove(2, newpos - elec_.R[2]);
  ValueType ratio_2 = std::exp(j3->evalLogRatio(elec_, 2));
  elec_.rejectMove(2);

  elec_.makeMove(3, newpos - elec_.R[3]);
  ValueType ratio_3 = std::exp(j3->evalLogRatio(elec_, 3));
  elec_.rejectMove(3);
