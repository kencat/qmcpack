...
void SlaterDetOpt::check_index_sanity() const
{
  ...
  for (std::vector<std::pair<int, int>>::const_iterator it = m_act_rot_inds.begin(); it != m_act_rot_inds.end(); it++)
  {
    if (it->first >= it->second)
    {
      ...
      error_msg << "check_index_sanity found an active rotation index pair (" << it->first << "," << it->second
                << ") in which the first index was not smaller than the second";
...
WaveFunctionComponent::RealType SlaterDetOpt::evaluate_matrices_from_scratch(ParticleSet& P, const bool all)
{
  ...
  qmcplusplus::MatrixOperators::transpose(m_orb_val_mat,
...
///
///////////////////////////////////////////////////////////////////////////////////////////////////
void SlaterDetOpt::restore(int iat) { curRatio = 1.0; }
                                   ...
                                   const RealType* const d2Bchi)
{
  ...
  {
    std::stringstream error_msg;
    error_msg << "supplied number of linear combinations (" << nl << ") does not match that held internally (" << m_nlc
              << ") in add_derivatives";
    throw std::runtime_error(error_msg.str());
  }
                                        ...
                                        const RealType* const dBchi)
{
  ...
  if (nl != m_nlc)
  {
    std::stringstream error_msg;
    error_msg << "supplied number of linear combinations (" << nl << ") does not match that held internally (" << m_nlc
              << ") in LCOrbitalSetOptTrialFunc::add_grad_derivatives";
    throw std::runtime_error(error_msg.str());
  }
  ...
  {
    std::stringstream error_msg;
    error_msg << "nl (" << nl << ") does not match size of m_hder_mat (" << m_hder_mat.size()
              << ") in LCOrbitalSetOptTrialFunc::add_grad_derivatives";
    throw std::runtime_error(error_msg.str());
  }
                                           ...
                                           std::vector<RealType>& dgradlogpsi)
{
  ...
  if (myVars.size() != m_act_rot_inds.size())
  {
    std::stringstream error_msg;
    error_msg << "mismatch between myVars.size() (" << myVars.size() << ") and m_act_rot_inds.size() ("
              << m_act_rot_inds.size() << ") in LCOrbitalSetOptTrialFunc::evaluateGradDerivatives";
    throw std::runtime_error(error_msg.str());
  }
