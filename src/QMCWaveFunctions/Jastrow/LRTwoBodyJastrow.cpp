...
{
LRTwoBodyJastrow::LRTwoBodyJastrow(ParticleSet& p) : NumPtcls(0), NumSpecies(0), skRef(0)
{
  Optimizable = false;
  NumSpecies  = p.groups();
  skRef       = p.SK;
...
void LRTwoBodyJastrow::resetTargetParticleSet(ParticleSet& P) { skRef = P.SK; }

LRTwoBodyJastrow::RealType LRTwoBodyJastrow::evaluateLog(ParticleSet& P,
...
LRTwoBodyJastrow::ValueType LRTwoBodyJastrow::LogRatio(ParticleSet& P, int iat)
{
  ...
  for (int ks = 0; ks < MaxKshell; ks++)
  {
    ...
    for (; ki < Kshell[ks + 1]; ki++)
    {
      ...
      dd += c * (c + (*rhok_r_ptr) - (*eikr_r_ptr)) + s * (s + (*rhok_i_ptr) - (*eikr_i_ptr));
  ...
  for (int ks = 0; ks < MaxKshell; ks++)
  {
    ...
    {
      sincos(dot(kpts[ki], pos), &s, &c);
      dd += c * (c + (*rhok_ptr).real() - (*eikr_ptr).real()) + s * (s + (*rhok_ptr).imag() - (*eikr_ptr).imag());
    }
    curVal += Fk_symm[ks] * dd;
  }
#endif
...
LRTwoBodyJastrow::ValueType LRTwoBodyJastrow::ratioGrad(ParticleSet& P, int iat, GradType& g)
{
  NeedToRestore = true;
  const KContainer::VContainer_t& kpts(P.SK->KLists.kpts_cart);
  {
...
LRTwoBodyJastrow::GradType LRTwoBodyJastrow::evalGrad(ParticleSet& P, int iat)
{
  NeedToRestore = true;
  const KContainer::VContainer_t& kpts(P.SK->KLists.kpts_cart);
  {
...
void LRTwoBodyJastrow::registerData(ParticleSet& P, WFBufferType& buf)
{
...
LRTwoBodyJastrow::RealType LRTwoBodyJastrow::updateBuffer(ParticleSet& P, WFBufferType& buf, bool fromscratch)
{
  LogValue = evaluateLog(P, P.G, P.L);
#if defined(USE_REAL_STRUCT_FACTOR)
  for (int iat = 0; iat < NumPtcls; iat++)
